...
import { Printer, Edit3, Loader2, Save, Trash2, Eye, User, CreditCard, History, X, RotateCcw } from "lucide-react"
                          ...
                          <Dialog open={!!viewingActor && viewingActor.id === actor.id} onOpenChange={(open) => !open && setViewingActor(null)}>
                            ...
                            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                              {viewingActor && (
                                <>
                                  <DialogHeader>
                                    <DialogTitle className="text-2xl font-black text-primary uppercase">Detail Pelaku Usaha</DialogTitle>
                                  </DialogHeader>
                                  <div className="grid gap-6 py-4">
                                    ...
                                    <section className="space-y-4">
                                      <div className="flex items-center gap-2 text-primary font-black text-sm uppercase"><CreditCard className="w-4 h-4" /> Rekening</div>
                                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-muted/30 p-4 rounded-xl">
                                        {[
                                          { label: "Bank", value: viewingActor.bankName },
                                          { label: "No. Rek", value: viewingActor.bankNumber },
                                          { label: "Pemilik", value: viewingActor.bankOwner }
                                        ].map((item, i) => (
                                          <div key={i} className="space-y-1">
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase">{item.label}</p>
                                    ...
                                    <section className="space-y-4">
                                      <div className="flex items-center gap-2 text-primary font-black text-sm uppercase"><History className="w-4 h-4" /> Audit</div>
                                      <div className="bg-slate-50 p-4 rounded-xl text-sm font-bold flex justify-between">
                                        <span>Input Oleh: <span className="text-primary">{viewingActor.createdBy || "System"}</span></span>
                                        <span>Daftar: {viewingActor.createdAt ? new Date(viewingActor.createdAt).toLocaleDateString('id-ID') : "-"}</span>
                                      </div>
                                    </section>
                                  </div>
                                </>
