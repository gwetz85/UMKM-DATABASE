...
export default function ChatMonitoring() {
  ...
  const handleDeleteOffline = async (id: string) => {
    if (!window.confirm('Hapus pesan offline ini?')) return;
    if (!database) return;

    try {
      deleteDocumentNonBlocking(ref(database, `offline_messages/${id}`));
  ...
  useEffect(() => {
    ...
    const unsubscribeOffline = onValue(offlineRef, (snapshot) => {
      const list: any[] = [];
      snapshot.forEach(child => {
        list.push({ id: child.key, ...child.val() });
      });
      setOfflineMessages(list.sort((a, b) => b.timestamp - a.timestamp));
    });
    ...
    <div className="max-w-7xl mx-auto p-8">
      <div className="mb-10">
        ...
        <div className="flex gap-2 mt-6">
          ...
          >
            Pesan Offline {offlineMessages.length > 0 && <span className="ml-2 bg-red-500 text-white px-2 py-0.5 rounded-full text-[10px]">{offlineMessages.length}</span>}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[350px_1fr] gap-6 h-[70vh]">
        ...
        <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 overflow-y-auto">
          {viewMode === 'live' ? (
            ...
            <>
              <h3 className="text-lg font-bold mb-4 px-2 text-slate-800">Daftar Form Offline</h3>
              {offlineMessages.length === 0 && <div className="text-center p-8 text-muted-foreground">Tidak ada pesan offline.</div>}
              
              {offlineMessages.map(msg => (
                <div
                  key={msg.id}
                  onClick={() => setSelectedOfflineId(msg.id)}
                  className={`p-4 rounded-2xl cursor-pointer mb-2 transition-all duration-200 border-2 ${
                    selectedOfflineId === msg.id 
                      ? 'border-primary bg-primary/10 shadow-sm' 
                      : 'border-transparent hover:bg-slate-50'
                  }`}
                >
                  <div className="font-bold text-sm mb-1 text-slate-800">{msg.name}</div>
          ...
          {viewMode === 'live' ? (
            ...
            selectedOfflineId ? (
              <>
                <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  <div>
                    <h3 className="font-bold text-slate-800 text-lg">Detail Pesan Offline</h3>
                    <span className="text-xs text-primary font-black uppercase tracking-widest">{offlineMessages.find(m => m.id === selectedOfflineId)?.name}</span>
                  </div>
                  {isAdmin && !isMonitoring && (
                    <button 
                      onClick={() => handleDeleteOffline(selectedOfflineId)}
                      className="bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-4 py-2 rounded-xl flex items-center gap-2 font-bold text-xs transition-colors"
                    >
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nama Lengkap</div>
                      <div className="text-lg font-bold text-slate-800">{offlineMessages.find(m => m.id === selectedOfflineId)?.name}</div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nomor Ponsel (WA)</div>
                      <div className="text-lg font-bold text-primary">{offlineMessages.find(m => m.id === selectedOfflineId)?.phone}</div>
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Isi Pesan</div>
                    <div className="bg-slate-50 p-6 rounded-3xl border-2 border-slate-100 text-slate-700 font-medium text-lg leading-relaxed shadow-inner">
                      "{offlineMessages.find(m => m.id === selectedOfflineId)?.message}"
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-100 flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    <span>Dikirim pada: {new Date(offlineMessages.find(m => m.id === selectedOfflineId)?.timestamp).toLocaleString('id-ID')}</span>
                    <span>Melalui: {offlineMessages.find(m => m.id === selectedOfflineId)?.userName || 'User Anonim'}</span>
                  </div>
                </div>
              </>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
                ...
                <p className="font-medium text-slate-500">Pilih pesan offline untuk melihat isi lengkap.</p>
              </div>
            )
          )}
        </div>
