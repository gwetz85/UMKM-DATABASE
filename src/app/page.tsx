  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="bg-white px-4 py-2 rounded-2xl border border-muted shadow-sm flex items-center gap-3">
        ...
        {stats.map((stat) => (
          <Card key={stat.name} className="border-none shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden bg-white">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{stat.name}</CardTitle>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="border-none shadow-sm bg-white lg:col-span-2">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-sm font-bold">
                <span className="text-slate-600">Menunggu Verifikasi Rekening Bank</span>
        ...
        <Card className="border-none shadow-sm bg-white overflow-hidden relative">
          <CardHeader>
            <CardTitle className="text-lg font-bold">Kategori UMKM</CardTitle>
          </CardHeader>
          <CardContent className="flex flex-col gap-6 justify-center h-full pb-10">
            <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <div className="flex flex-col">
                <span className="text-xs font-bold text-muted-foreground uppercase">Kuliner</span>
                <span className="text-2xl font-black text-primary">{allData?.filter(d => d.businessCategory === "Kuliner").length}</span>
              </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <div className="flex flex-col">
                <span className="text-xs font-bold text-muted-foreground uppercase">Bukan Kuliner</span>
                <span className="text-2xl font-black text-slate-700">{allData?.filter(d => d.businessCategory === "Bukan Kuliner").length}</span>
              </div>
              <div className="p-3 bg-white rounded-xl shadow-sm">
