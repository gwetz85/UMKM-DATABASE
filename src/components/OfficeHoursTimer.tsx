...
export function OfficeHoursTimer({ large = false }: { large?: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-3 ${large ? "px-6 py-4 rounded-3xl" : "px-3 py-1.5 md:px-4 md:py-2 rounded-2xl"} border transition-all duration-500 shadow-lg bg-white/95 backdrop-blur-md ${status.colorClass.split(' ').filter(c => !c.startsWith('bg-')).join(' ')}`}>
